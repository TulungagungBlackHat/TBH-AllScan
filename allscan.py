#!/usr/bin/env python3
# TBH-AllScan v3.6 Pro
import socket, requests, argparse, json
from datetime import datetime
import time
BANNER="""\033[91m╔════════════════════════════════════════╗
\033[91m║ \033[97mTBH-AllScan v3.6 Pro \033[91m║
...
def allscan(url):
    ...
    print(f"[✓] Done {elapsed}s | Risk Low (3) | v3.6 Pro")
    return report
def main():
    print(BANNER)
    parser=argparse.ArgumentParser(description="v3.6")
    ...
    if args.html: open(args.html,'w').write(f"<html><body><h1>v3.6 Pro {report['target']}</h1><pre>{json.dumps(report,indent=2)}</pre></body></html>"); print(f"[✓] HTML: {args.html}")
