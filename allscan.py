#!/usr/bin/env python3
# TBH-AllScan v4.4 Pro
import socket, requests, argparse, json
from datetime import datetime
import time
BANNER="""\033[91m╔════════════════════════════════════════╗
\033[91m║ \033[97mTBH-AllScan v4.4 Pro \033[91m║
\033[91m╚════════════════════════════════════════╝\033[0m"""
def allscan(url):
    import urllib.parse
    ...
    start=time.time()
    print(f"[*] {domain} ({ip})")
    report={"target":domain,"ip":ip,"url":url,"time":str(datetime.now()),"version":"4.4 Pro","bugs":[]}
    ...
    print(f"[✓] Done {elapsed}s | Risk Low (3) | v4.4 Pro")
    return report
def main():
    print(BANNER)
    parser=argparse.ArgumentParser(description="v4.4")
    ...
    if args.html: open(args.html,'w').write(f"<html><body><h1>v4.4 Pro {report['target']}</h1><pre>{json.dumps(report,indent=2)}</pre></body></html>"); print(f"[✓] HTML: {args.html}")
