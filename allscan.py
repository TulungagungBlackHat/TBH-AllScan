...
BANNER = """\033[91m╔════════════════════════════════════════╗
\033[91m║ \033[97mTBH-AllScan v2.3 Pro \033[91m- Severity Color \033[91m║
\033[91m║ \033[90mTulungagung Black Hat | uchil404     \033[91m║
\033[91m╚════════════════════════════════════════╝\033[0m"""

CVE_DB = {80: "CVE-2023-44487", 443: "CVE-2023-44487", 445: "CVE-2017-0144 EternalBlue"}

def sev_color(s):
    return {"High":"#ff0000","Medium":"#ff8c00","Low":"#ffcc00","Info":"#58a6ff"}.get(s,"#c9d1d9")

def check_headers(url):
    try:
        r=requests.get(url,timeout=5,headers={'User-Agent':'TBH-AllScan/2.3'})
...
def check_ports(ip):
    ...
    for p in [80,443,8080,8443]:
        ...
        s.close()
    bug={"severity":"Info","title":"Open Ports + CVE","ports":open_ports,"fix":"Patch & tutup"} if open_ports else None
...
def main():
    print(BANNER)
    parser=argparse.ArgumentParser(description="v2.3")
    ...
    if args.html:
        rows="".join([f"<tr><td>{b['tool']}</td><td style='background:{sev_color(b['severity'])};color:#000;font-weight:bold'>{b['severity']}</td><td>{b['title']}</td><td>{b.get('fix','')}</td></tr>" for b in report["bugs"]])
        html=f"<html><head><style>table{{border-collapse:collapse;width:100%}}th,td{{border:1px solid #30363d;padding:8px}}th{{background:#ff0000;color:#fff}}</style></head><body style='background:#0d1117;color:#c9d1d9;padding:20px;font-family:monospace'><h1>TBH-AllScan v2.3 Pro {report['target']}</h1><table><tr><th>Tool</th><th>Severity</th><th>Bug</th><th>Fix</th></tr>{rows}</table><pre>{json.dumps(report,indent=2)}</pre></body></html>"
        open(args.html,'w').write(html); print(f"[✓] HTML: {args.html} (color)")
