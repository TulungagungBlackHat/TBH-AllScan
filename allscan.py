...
BANNER = """\033[91m╔════════════════════════════════════════╗
\033[91m║ \033[97mTBH-AllScan v3.3 Pro \033[91m- Progress Bar   \033[91m║
...
def allscan(url):
    ...
    for i,name in enumerate(checks,1):
        ...
        # Simulate check
        if name=="Headers": report["bugs"].append({"tool":name,"severity":"Low","title":"Missing Headers"})
        elif name=="Ports": report["bugs"].append({"tool":name,"severity":"Info","title":"Open Ports"})
    print()
    ...
    print(f"[Time] {elapsed}s | Risk Low ({score}) | Bugs {len(report['bugs'])}/10")
    return report

def main():
    print(BANNER)
    parser=argparse.ArgumentParser(description="v3.3")
    ...
    if args.html:
        html=f"<html><body style='background:#0d1117;color:#c9d1d9;padding:20px;font-family:monospace'><h1>v3.3 Pro {report['target']} | {report['elapsed']}s</h1><pre>{json.dumps(report,indent=2)}</pre></body></html>"
